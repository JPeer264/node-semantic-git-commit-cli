  ...
  const questionsList = [
    {
      ...
      source: (answer, input) => {
        const filteredChoices = choicesList.filter((choice) => (
          // check if input matches value
          choice.value.toLowerCase().includes(input)
          // check if input matches any key
          || (choice.key || []).some((key) => key.toLowerCase().includes(input))
        ));

        return Promise.resolve(input ? filteredChoices : choicesList);
      },
    },
    {
      type: 'input',
