...
export const combineTypeScope = (type, scope, delimiter = ':') => {
  let thisType = type;
  ...
  if (thisScope.length > 0) {
    ...
    thisType = `${thisType} ${thisScope}${trimmedDelimiter}`;
  } else if (
    thisType.charAt(thisType.length - 1) !== trimmedDelimiter
    || thisType.charAt(thisType.length - 1) !== originalDelimiter
  ) {
...
export const formatMessage = (answers, argv, config) => {
  ...
  const type = combineTypeScope(correctType, combinedAnswers.scope, delimiter);
